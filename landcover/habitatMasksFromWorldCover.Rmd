...
The histograms were generated on Google Earth Engine (GEE), using the following script: https://code.earthengine.google.com/d7456474434eb293662f2373e3a7f549?noload=1
...
Grid-wise histograms (column values are counts of 10mx10m pixels) were first generated on GEE, for all grid cells at once.

```{r message=FALSE, warning=FALSE}
gridHists <- geojson_sf("data/histograms/wc2021Grd25kmHist_allGrids.geojson") %>%
...
totalExtents <- gridHists %>% 
  filter(GRID_G1 != "8671") %>% # problem grid in WB: a GeometryCollection leading to a "s2_unary_union() for multidimensional collections not implemented" error
  mutate(one = bare + grassland + shrubland + savanna) %>% 
...
```{r}
st_write(gridwiseHabMaskAddedAllSt %>% st_as_sf(), "outputs/soibV2GridsIN_habMasksAdded.geojson")
```
